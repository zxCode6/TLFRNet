...
model_trained = './results/SGD_Cosine_Lr0.05_DN4_Conv64F_Local_Epoch_50_CubBirds_84_84_5Way_1Shot/'

parser = argparse.ArgumentParser()
parser.add_argument('--dataset_dir', default='./dataset/CubBirds', help='/miniImageNet--ravi')
parser.add_argument('--data_name', default='CubBird', help='miniImageNet|StanfordDog|StanfordCar|CubBird_2011')
...
parser.add_argument('--test_aug', action='store_true', default=False,
                    help='Perform data augmentation or not during test')
#  Few-shot parameters  #
...
parser.add_argument('--shot_num', type=int, default=4, help='the number of shot')
...
parser.add_argument('--aug_shot_num', type=int, default=20,
                    help='the number of augmented support images of each class during test')
parser.add_argument('--neighbor_k', type=int, default=3, help='the number of k-nearest neighbors')
#  Other parameters   #
parser.add_argument('--lr', type=float, default=0.0006, help='initial learning rate')
parser.add_argument('--cosine', type=bool, default=True, help='using cosine annealing')
parser.add_argument('--lr_decay_epochs', type=list, default=[20, 50], help='where to decay lr, can be a list')
...
def test(test_loader, model, criterion, epoch_index, best_prec1, F_txt):
    ...
    for episode_index, (query_images, query_targets, support_images, support_targets) in enumerate(test_loader):
        ...
        target = torch.cat(query_targets, 0).long().cuda()
...
if __name__ == '__main__':
    ...
    model = FewShotNet.define_model(encoder_model=opt.encoder_model, classifier_model=opt.classifier_model,
                                    norm='batch',
